...
/*
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, 'https://get.roost.me/api/push');
	curl_setopt($ch, CURLOPT_USERPWD, "4c81eda8d1c6482b80fc49459d4c2a33:a6053eedfade4e9ba40260e259b6add8");
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
	curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
	curl_exec($ch);
...
$(document).ready(function(){
	...
	// устанавливаем курсор в необходимое положение
...
//		preview = preview.replace(/<p><\/p>/g, '');
		...
		// ссылки в превью зоне внизу с id чтобы можно было переходить
...
for (var i = 0; i < files.length; i++) {
    ...
    previewImg(files[i],numberOfImages);
...
*/
    ...
    //formData.append('hash','<?php echo $nameHash;?>');
...
$('#mainImage').change(function(e){
		...
		console.log('imageAdded '+ file.name);		
});
